...
Original file is located at
    https://colab.research.google.com/drive/1JuzoDIy5wsBvwLNWhaQd-ILbzZzPcbbE
"""
...
sales_order_detail_path = "sales-order-detail.csv"
sales_order_header_path = "sales-order-header.csv"
...
base_delta_path = "/content/delta_tables"
...
def save_table(df, table_name: str, mode: str = "overwrite"):
    (
        ...
        .save(f"{base_delta_path}/{table_name}")
    )

def read_table(table_name: str):
    return spark.read.format("delta").load(f"{base_delta_path}/{table_name}")
...
# -----------------------------
orders_with_product = (
    read_table("publish_orders").alias("o")
    .join(
        read_table("publish_product")
        .select("ProductID", "Color", "ProductCategoryName")
        .alias("p"),
        on="ProductID",
        how="left"
    )
